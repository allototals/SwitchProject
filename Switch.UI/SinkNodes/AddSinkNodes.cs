...
namespace Switch.UI
{
    public class AddSinkNodes:EntityUI<SinkNode>
    {
        public AddSinkNodes()
        {
            UseFullView();
            AddNorthSection()
                ...
                .SubmitTo(x =>
                {
                    ...
                    try
                    {
                        new GenericService<SinkNode>().Create(x);
                    ...
                    catch (Exception ex)
                    {
                       // PANE.ERRORLOG.ErrorLogger.Log(ex); 
                        x.Err_Msg = string.Format("An error occured while saving SinkNode: {0}", ex.Message);
                        result = false;
                    }
                    return result;
                })
                .OnFailureDisplay(x => x.Err_Msg)//"Batch Upload Failed! Please check the file and try again.")
                .OnSuccessRedirectTo("SinkNodeList.aspx",x=>x.Id)
                .OnSuccessDisplay("SinkNode successfully created")
             });
        }
    }
}
